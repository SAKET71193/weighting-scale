...
{
   size_t retVal; 
   // Initialise the Wire1 library.
   Wire1.begin();
   
   Wire1.beginTransmission ( _i2cAddress );
    Wire1.write ( (byte)0x0 );   // Send command to the display
   Wire1.write ( FUNC_SET_TBL0 );
   delay (10);
   Wire1.write ( FUNC_SET_TBL1 );
   delay (10);
   Wire1.write ( 0x14 );  // Set BIAS - 1/5
   Wire1.write ( 0x73 );  // Set contrast low byte
   Wire1.write ( 0x5E );  // ICON disp on, Booster on, Contrast high byte 
   Wire1.write ( 0x6D );  // Follower circuit (internal), amp ratio (6)
   Wire1.write ( 0x0C );  // Display on
   Wire1.write ( 0x01 );  // Clear display
   Wire1.write ( 0x06 );  // Entry mode set - increment
    _status = Wire1.endTransmission ();
   if ( _status == 0 )
   {
      _initialised = true;
   }
...
size_t ST7036::write(const uint8_t *buffer, size_t size)
{
   ...
   if ( _initialised )
   {
      Wire1.beginTransmission ( _i2cAddress );
      Wire1.write ( RAM_WRITE_CMD );
      Wire1.write ( (uint8_t *)buffer, size );
      _status = Wire1.endTransmission ();
      delay(_charDelay);
   }
}
...
void ST7036::load_custom_character (uint8_t char_num, uint8_t *rows)
{
   ...
   if ( _initialised )
   {
      ...
      if ( char_num < MAX_USER_CHARS )
      {
         ...
         Wire1.beginTransmission ( _i2cAddress );
         Wire1.write ( DISP_CMD );
         Wire1.write ( FUNC_SET_TBL0 ); // Function set: 8 bit, 2 line display 5x8, funct tab 0
         delay ( _cmdDelay );
         
         // Set CGRAM position to write
         Wire1.write ( RAM_WRITE_CMD + (PIXEL_ROWS_PER_CHAR * char_num) ); 
         _status = Wire1.endTransmission();
